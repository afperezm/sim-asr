...
def get_resources_list(db_conn, db_name, audios_only=False, identifiers_list=None):
    ...
    if audios_only:
        result = db_conn[db_name].records.aggregate(audio_pipeline)
    else:
...
def copy_audio_records_with_rollback(ssh_client, resource):
    """Copies audio records files and rollback if copy fails."""
    ...
    for idx, audio_record in enumerate(audio_records):
        ...
        copy_result = copy_audio_records(ssh_client, resource_id, audio_record)
...
def copy_audio_records(ssh_client, resource_id, audio_record):
    """Copies audio records files from a remote server through SSH."""
...
def copy_resources(ssh_client, resources_list, max_workers, audios_only=False):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        if audios_only:
            futures = {
                executor.submit(copy_audio_records_with_rollback, ssh_client=ssh_client, resource=resource): resource[
                    '_id'] for resource in resources_list}
        else:
            futures = {
                executor.submit(copy_records_with_rollback, ssh_client=ssh_client, resource=resource): resource['_id']
...
def main():
    ...
    resources_list = get_resources_list(db_conn, PARAMS.mongo_db, PARAMS.audios_only, PARAMS.resources_list)
...
def parse_args():
    ...
    parser.add_argument("--audios_only", help="Flag to indicate copy audios only", action='store_true')
    parser.add_argument("--resources_list", help="List of resources to copy", nargs='+', default=[])
